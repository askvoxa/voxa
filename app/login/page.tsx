...
export default function LoginPage() {
  ...
  // Mock do Social Login
  const handleSocialLogin = async (provider: 'google' | 'twitch' | 'email') => {
    setIsLoading(true)
    try {
      // Simula um delay de rede
      await new Promise(res => setTimeout(res, 1000))
      
      // Redireciona para o dashboard simulando sucesso
      router.push('/dashboard')
    } catch (error) {
      console.error('Error logging in:', error)
      alert('Erro ao fazer login simulado')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#050505] p-4 text-white relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-[#111] rounded-[32px] border border-white/10 p-8 shadow-2xl text-center relative z-10">
        ...
        {/* Security / Protection Badge Segment */}
